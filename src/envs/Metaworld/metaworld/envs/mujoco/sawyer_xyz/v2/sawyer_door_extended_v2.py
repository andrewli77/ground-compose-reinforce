...
class SawyerDoorExtendedEnvV2(SawyerXYZEnv):
    def __init__(
        self,
    ...
    def model_name(self) -> str:
        return full_v2_path_for("sawyer_xyz/sawyer_door_extended.xml")
    ...
    def evaluate_state(
        ...
        # print(abs(obs[4] - self._target_pos[0]))
    ...
    def _get_pos_objects(self) -> npt.NDArray[Any]:
        handle_pos = self.data.geom("handle").xpos.copy()
        box_pos = self.data.geom("objGeom").xpos.copy()
        return np.concatenate((handle_pos, box_pos))

    def _get_quat_objects(self) -> npt.NDArray[Any]:
        handle_quat = Rotation.from_matrix(
            self.data.geom("handle").xmat.reshape(3, 3)
        ).as_quat()
        box_quat = Rotation.from_matrix(
            self.data.geom("objGeom").xmat.reshape(3, 3)
        ).as_quat()
        return np.concatenate((handle_quat, box_quat))
    ...
    def compute_reward(
        ...
        reward_grab = SawyerDoorExtendedEnvV2._reward_grab_effort(actions)
        reward_steps = SawyerDoorExtendedEnvV2._reward_pos(obs, theta)
